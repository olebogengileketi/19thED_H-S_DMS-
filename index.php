<?php
// Load error handler first (before anything else)
...
try {
    $totalMembers   = safeQuery($pdo, "SELECT COUNT(*) FROM members")->fetchColumn();
    $totalChurches  = safeQuery($pdo, "SELECT COUNT(*) FROM churches WHERE status = 'active'")->fetchColumn();
    $totalAreas     = safeQuery($pdo, "SELECT COUNT(*) FROM areas")->fetchColumn();
    $totalConfs     = safeQuery($pdo, "SELECT COUNT(*) FROM conferences")->fetchColumn();
    $totalEvents    = safeQuery($pdo, "SELECT COUNT(*) FROM events")->fetchColumn();

    $components = safeQuery($pdo, "
        SELECT
    ...
    $hasJoinedYpd  = (bool)safeQuery($pdo, "SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='members' AND COLUMN_NAME='joined_ypd'")->fetchColumn();
    $hasFullChurch = (bool)safeQuery($pdo, "SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='members' AND COLUMN_NAME='full_member_of_church'")->fetchColumn();
    $hasOccStatus  = (bool)safeQuery($pdo, "SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='members' AND COLUMN_NAME='occupational_status'")->fetchColumn();

    $ypdYes = $hasJoinedYpd  ? (int)safeQuery($pdo, "SELECT SUM(joined_ypd='Yes') FROM members")->fetchColumn() : 0;
    $ypdNo  = $hasJoinedYpd  ? (int)safeQuery($pdo, "SELECT SUM(joined_ypd='No' OR joined_ypd IS NULL) FROM members")->fetchColumn() : 0;

    $fullYes = $hasFullChurch ? (int)safeQuery($pdo, "SELECT SUM(full_member_of_church='Yes') FROM members")->fetchColumn() : 0;
    $fullNo  = $hasFullChurch ? (int)safeQuery($pdo, "SELECT SUM(full_member_of_church='No' OR full_member_of_church IS NULL) FROM members")->fetchColumn() : 0;

    $occRows = [];
    if ($hasOccStatus) {
        $occRows = safeQuery($pdo, "
    ...
    logError("Error fetching dashboard stats: " . $e->getMessage(), "DASHBOARD");
    ...
    $totalConfs = 0;
    $totalEvents = 0;
    $components = ['MB' => 0, 'AS_' => 0, 'Y' => 0, 'YA' => 0];
    $genders = ['Male' => 0, 'Female' => 0];
    $bestEvent = ['event_name' => 'N/A', 'total_attendance' => 0];
    $lowestEvent = ['event_name' => 'N/A', 'total_attendance' => 0];
    $latestMedia = [];
    $latestStories = [];
    $hasJoinedYpd = false;
    $hasFullChurch = false;
